...
import com.app.ezride.repositories.BookingDetailsRepo;
...
public class RideServiceImpl implements IRideService {
    ...
    public void bookBus(int rideId, int customerId, int numberOfSeats, String seatNumbers) {
        int seats;
        Ride ride = rideRepo.findById(rideId).orElseThrow(() -> new RuntimeException("Ride not found"));
        Customer customer = customerRepo.findById(customerId).orElseThrow(()->new RuntimeException("Customer not found"));
        if (ride.getCapacity() > 0) {
        ...
        BookingsDetails bookingsDetails = new BookingsDetails(customer.getId(),seatNumbers);
        ride.addBookingDetails(bookingsDetails);
        bookingDetailsRepo.save(bookingsDetails);
